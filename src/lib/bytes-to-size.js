...
export const bitsToSize = (bits) => {
  const sizes = ["b", "Kb", "Mb", "Gb", "Tb"];
  if (bits === 0) return "0 b";
  const i = parseInt(Math.floor(Math.log(bits) / Math.log(1024)), 10);
  if (i === 0) return `${bits.toFixed(1)} ${sizes[i]}`;
  return `${(bits / 1024 ** i).toFixed(1)} ${sizes[i]}`;
}

export const intToSize = (num) => {
  const sizes = ["", "K", "M", "B", "T"];
  if (num === 0) return "0";
  const i = parseInt(Math.floor(Math.log(num) / Math.log(1000)), 10);
  if (i === 0) return `${num.toFixed(1)} ${sizes[i]}`;
  return `${(num / 1000 ** i).toFixed(1)} ${sizes[i]}`;
}
